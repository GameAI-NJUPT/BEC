...
class Agent:
    def __init__(self, env, tiling_specs, layers=5, features=6000, gamma=1.
                 , epsilon=0.01, alpha=0.005, zeta=0.0001, beta=0.0001, coherenceLearning=False):
        self.env = env
    ...
    def TDClearning_new(self, observation, action, reward, next_observation, terminated, isEpsilon, isOffPolicy=False):
        ...
        # print(phiPsi)
        self.w[features] += self.alpha * (td_error - self.omga)
        self.w[next_features] -= self.alpha * phiPsi * self.gamma
        # self.omga += self.gy * (td_error - self.omga)
        self.psi[features] += self.zeta * ((td_error - self.omga) - phiPsi)
        self.omga += self.beta * (td_error - self.omga)
    ...
    def test(self, render=False):
        steps = []
        for episode in range(10):  # 没训练50局的时候，测试200局，返回这200局对应的平局步伐
            observation, _ = self.env.reset()  # 初始状态
