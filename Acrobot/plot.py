...
def plot_data0(
        path, step, weight
):  # path:路径；  step:截取数据长度； plot_step：截取数据步长； weight:平滑权重
...
if __name__ == "__main__":
    ...
    path1 = "../results/acrobot/Q.npy"  # Q
    path2 = "../results/acrobot/GQ.npy"  # GQ
    path3 = "../results/acrobot/CQ.npy" #CQ
    path4 = "../results/acrobot/CGQ.npy"  # CGQ

    # 修改 step 值以匹配实际数据长度
    step = 1500
    sample_rate = 50  # 控制采样疏密程度的核心参数

    step1, smoothed1, mean1, std1 = plot_data0(path1, step, 0.9)
    step2, smoothed2, mean2, std2 = plot_data0(path2, step, 0.9)
    step3, smoothed3, mean3, std3 = plot_data0(path3, step, 0.9)
    step4, smoothed4, mean4, std4 = plot_data0(path4, step, 0.9)
